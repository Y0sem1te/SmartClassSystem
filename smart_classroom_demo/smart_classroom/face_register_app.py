        ...
        while ret and self.running:
            try:
                ...
                if self.is_adding_new_name:
                    ...
                    face_keypoints = np.array(face_landmarks, dtype=np.float64)
                    # 边界范围
                ...
                for (x1, y1, x2, y2), (label, prob) in zip(face_locations, silent_face_detections):
                    ...
                    left, top, right, bottom = fontText.getbbox(result_text)
                    draw.text((x1, y1 + top-bottom), result_text, color, fontText)
                # 绘制文字提示
                if self.center_tips_text_ckb.isChecked():
                    l,t,r,b = self.fontTextBig.getbbox(tips_text)
