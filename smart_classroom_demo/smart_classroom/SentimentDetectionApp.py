...
from pipeline_module.emoji_module import SentimentEmojiModule
...
from pipeline_module.sentiment_modules import SentimentModule
...
sentiment_weight='./weights/sentiment.pth'
device = 'cuda'

from ui.sentiment_analysis import Ui_SentimentAnalysis  # 确保路径正确
...
class SentimentDetectionApp(QWidget, Ui_SentimentAnalysis):
    ...
    def __init__(self, parent=None):
        ...
        self.show_box_ckb.stateChanged.connect(self.change_frame)
    ...
    def open_source(self, source):
        ...
        def open_source_func(self):
            ...
            self.opened_source = TaskSolution() \
                ...
                .set_next_module(SentimentModule(sentiment_weight, device=device)) \
                .set_next_module(SentimentEmojiModule(lambda d: self.push_frame_signal.emit(d),
    ...
    def change_frame(self):
        try:
            ...
            frame = data.get_draw_frame(show_raw=False,
                                        show_locations=True,
                                        threshold=1)
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            frame = cv2.resize(frame, (self.video_screen.width() - 9, self.video_screen.height() - 9))  # 调整图像大小
    ...
    def open_face_bank(self, bank_name=None):
        try:
            ...
            face_bank_dir =face_bank_base_dir
            # print("shit!",face_bank_dir)
    ...
    def get_known_faces_data(facebank):
        try:
            ...
            known_faces_data = [(name, read_encoding_json2npy(
                ...
                # os.path.join(facebank, 'encoding.json')
            ), read_img_from_cn_path(
                os.path.join(facebank, name, 'face.jpg')
                # os.path.join(facebank, 'face.jpg')
