...
class YoloV5Detector:
    def __init__(self, weights, device):
        ...
        self.max_det = 1000
        self.classes = [0]
        # self.transformer = transforms.Compose([transforms.ToTensor(),transforms.Resize((640,480))])
    ...
    def preprocess_img(self, img):
        ...
        # img = img[:, :, :, ::-1].transpose(0, 3, 1, 2) / 255  # BGR to RGB, to bsx3x416x416
        # img = np.ascontiguousarray(img)
        # print(self.transformer(img[:, :, ::-1].copy()).unsqueeze(0).to(self.device, dtype=torch.float32).shape)
