...
class PnPPoseEstimator:
    ...
    def draw_axis(self, img, R, t):
        ...
        img = img.copy()
        # img = cv2.resize(img.copy(), (480, 640))
        cv2.line(img, tuple(axisPoints[3].ravel().astype(int)), tuple(
            axisPoints[0].ravel().astype(int)), (255, 0, 0), 3)
        cv2.line(img, tuple(axisPoints[3].ravel().astype(int)), tuple(
            axisPoints[1].ravel().astype(int)), (0, 255, 0), 3)
        cv2.line(img, tuple(axisPoints[3].ravel().astype(int)), tuple(
            axisPoints[2].ravel().astype(int)), (0, 0, 255), 3)
